defmodule ScenicTest.Scenic.PreReq do
  ...
  @clip_steps Application.get_env(:scenic_test, :config)[:clip_steps]
  ...
  def compute_mandelbrot({width,height} = screen, {x0,y0} = _start \\ {0,0}) do
      ...
      |> Enum.filter(fn {_,steps} -> steps > @clip_steps end)
  end

  def plot_mandelbrot(graph,screen, start \\ {0,0}) do
    points =
      compute_mandelbrot(screen,start)
  ...
  def map_to_spec(points, {width, height}) do
    points
    |> Enum.filter(fn {{x,y},steps} -> x < width and y < height and steps > @clip_steps end)
  ...
  defp check_mandelbrot(point, orig, step) do
    ...
    cond do
      magnitude(point) > 2.0 or step >= @step_size -> step
      magnitude(point) <= 2.0 and step >= @step_size -> step
      true -> point
        |> next_iter(orig)
